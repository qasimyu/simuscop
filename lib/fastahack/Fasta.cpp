...
void FastaIndex::readIndexFile(string fname) {
    ...
    if (indexFile.is_open()) {
		...
		size_t indx = name.find("chrom");
		if(indx == string::npos){
			indx = name.find("chr");
			if(indx != string::npos){
				name = name.substr(indx+3,name.size()-3);
			}
		}
		else{
			name = name.substr(indx+5,name.size()-5);
		}
...
string FastaReference::getSequence(string seqname) {
    ...
    if (usingmmap) {
        ...
        size_t bytes = fread(seq, sizeof(char), seqlen, file);
    }
    seq[seqlen] = '\0';
    char* pbegin = seq;
...
string FastaReference::getSubSequence(string seqname, int start, int length) {
    ...
    if (usingmmap) {
        ...
        size_t bytes = fread(seq, sizeof(char), (off_t) seqlen, file);
    }
    seq[seqlen] = '\0';
    char* pbegin = seq;
    char* pend = seq + (seqlen/sizeof(char));
...
string FastaReference::getFileName() {
    return filename;
}
